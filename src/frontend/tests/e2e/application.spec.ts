...
test('visitor can navigate, enter the ranking, and Boost without an account', async ({ page }) => {
  ...
  await page.route('http://localhost:5080/api/**', async route => {
    ...
    if (route.request().method() === 'POST' && url.pathname === '/api/creators') {
      const added = { ...ada, id: 'creator-2', username: 'grace', name: 'Grace Hopper', totalContributed: 12.5, dailyContributed: 12.5 }
      creators.unshift(added)
      return route.fulfill({ json: { creatorId: added.id, session: { id: 'mock-entry', confirmed: true } }, status: 201, headers: corsHeaders })
    }
