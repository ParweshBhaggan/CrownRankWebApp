...
export function BoostDialog({ creator, gateway, onClose, onConfirmed }: Props) {
  ...
  return <dialog ref={ref} className="boost-dialog" onClose={close} onCancel={(e) => { e.preventDefault(); close() }}>
    ...
    {ready ? <div className="boost-success" role="status"><span>✓</span><h2>Boost confirmed</h2><p>Your contribution was saved.</p><button className="primary-button" onClick={close}>Done</button></div> : <form onSubmit={submit} noValidate>
      <p className="eyebrow">Boost this creator</p><div className="boost-person"><img src={creator.imageUrl} alt="" /><div><h2>{creator.name}</h2><p>@{creator.username}</p></div></div>
      ...
      <button className="primary-button full" type="submit" disabled={busy}>{busy ? 'Preparing checkout…' : `${error ? 'Retry checkout' : 'Continue to checkout'} · ${formatCurrency(parseAmount(amount))}`}</button><small className="secure-note">Secure checkout · No account required</small>
    </form>}
  </dialog>
}
